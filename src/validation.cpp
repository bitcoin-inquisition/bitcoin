...
static script_verify_flags DepDiscourageFlags(const CBlockIndex* tip, const ChainstateManager& chainman, const std::vector<std::pair<Consensus::DeploymentPos,script_verify_flags>>& depflags)
{
    script_verify_flags result = SCRIPT_VERIFY_NONE;
...
bool MemPoolAccept::PolicyScriptChecks(const ATMPArgs& args, Workspace& ws)
{
    ...
    const script_verify_flags scriptVerifyFlags = STANDARD_SCRIPT_VERIFY_FLAGS | DepDiscourageFlags(m_active_chainstate.m_chain.Tip(), m_active_chainstate.m_chainman, {
        INQ_POLICY_CHECKS
    });
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev)
{
    ...
    // Check that no transactions are 64-byte (as that may cause the block to be malleable, see
    // https://gnusha.org/pi/bitcoindev/CAFp6fsGtEm9p-ZQF_XqfqyQGzZK7BS2SNp2z680QBsJiFDraEA@mail.gmail.com/) and
    // that all transactions are finalized.
